import { getInterfaceInfoByIdUsingGet } from '@/services/ggapi_backend/interfaceInfoController';
import { PageContainer } from '@ant-design/pro-components';
import { message,Descriptions, Form, Button, Input, Divider } from 'antd';
...
const Index: React.FC = () => {
  ...
  const onFinish = async (values: any) => {
    ...
    // 在开始调用接口之前,将 invokeloading设置为 true,表示正在加载中
    ...
    // 无论成功或失败,最后将invokeLoading设置为 false,表示加载完成
      ...
      <Card>
        {data ?(
          <Descriptions title={data.name} column={1}>
            <Descriptions.Item label="接口状态">{data.status ? '关闭' : '开启'}</Descriptions.Item>
